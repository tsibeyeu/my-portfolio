    ...
    <tbody>
        ...
        <tr>
            ...
            <td> <form action="{{route('trainingpackage.destroy')}}" method="POST">
                ...
                @method('DELETE')
                <input type="hidden" name="trainingpackage_id" value="{{$trainingPackage->id}}"/>
                <button  onclick="return confirm('Are you sure you want to delete this Training Package?')"  type="submit" class="btn btn-danger btn-sm delete btn-flat"><i class='fa fa-trash-alt'></i></</button>
            </form>
            </td>
