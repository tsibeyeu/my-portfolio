        ...
        <tr>
           ...
           @endforeach
            ...
            <td> <form action="{{route('member.destroy')}}" method="POST">
                ...
                @method('DELETE')
                <input type="hidden" name="member_id" value="{{$member->id}}"/>
                <button  onclick="return confirm('Are you sure you want to delete this Member?')"  class="btn btn-danger btn-sm delete btn-flat" type="submit" ><i class='fa fa-trash'></i></button>
            </form>
            </td>
