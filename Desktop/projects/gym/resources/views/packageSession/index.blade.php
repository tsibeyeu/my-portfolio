    ...
    <tbody>
        ...
        <tr>
            ...
            <td> <form action="{{route('packagesession.destroy')}}" method="POST">
                ...
                @method('DELETE')
                <input type="hidden" name="packageSession_id" value="{{$packageSession->id}}"/>
                <button  onclick="return confirm('Are you sure you want to delete this Package Session?')"  type="submit" class="btn btn-danger btn-sm delete btn-flat"><i class='fa fa-trash'></i></</button>
            </form>
            </td>
